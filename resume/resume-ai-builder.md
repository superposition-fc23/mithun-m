...
AI builder and platform engineer with 10 years shipping products from 0 to 1 to 10 at seed to Series E startups, 5 of them bootstrapping a SaaS company. I work on the API and data-model layer that lets one platform serve multiple products instead of one-off integrations: partnering with engineering on the architecture that unifies Support, Build, and Grow on one system-of-record at DevRev, and architecting a multi-source compute engine at Xoxoday (2B+ transactions, 5M+ users) serving both web and mobile. At Shoptree I built the earlier version of that instinct myself: a single data model and REST API marketplace (8+ integrations) so 300+ mid-sized and enterprise customers ran POS, inventory, and supply chain on one platform. I treat latency, reliability, and infra cost as one tradeoff surface, not three separate goals: sub-400ms voice AI priced 7x cheaper than the incumbent (MSPbots), sub-3s RAG at 10K+ queries/day (DevRev), 99% reliability while cutting infra spend 30% (Xoxoday), and sub-13s checkout over 2G (Shoptree). Daily Python across RAG, multi-agent systems, and computer vision.
...
- Selected the production voice pipeline over 4 candidates via controlled load testing, Deepgram+Twilio at P50 378ms / P95 437ms (46% headroom on the 800ms SLA); modeled unit economics against Deepgram STT/TTS and Azure GPT inference cost to price 7x below the incumbent.
