...
AI builder and platform engineer with 10 years shipping products from 0 to 1 to 10 at seed to Series E startups, 5 of them bootstrapping a SaaS company. I own the API and data-model layer that lets one platform serve multiple products instead of one-off integrations, and I treat latency, reliability, and infra cost as one tradeoff surface, not three separate goals. Recently took a real-time voice AI agent from prototype to alpha monetization solo, owning the pipeline, the evals, and the economics. Daily Python across RAG, multi-agent systems, and computer vision.
